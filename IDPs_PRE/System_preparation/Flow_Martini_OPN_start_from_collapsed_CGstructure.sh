...
#Martinize
$python $martinize -f OPN_AA.pdb -o PRO_topol.top -x trash.pdb -ff $FF -ff-dir $ffdir/martini_v3.0.0_proteins/force_fields/ -map-dir $ffdir/martini_v3.0.0_proteins/mappings/ 

rm trash.pdb
cp OPN_CG.pdb PRO_CG.pdb

$python $change_termbead molecule_0.itp

#Put protein in box
$gmx editconf -f PRO_CG.pdb -o PRO_CG.gro -bt dodecahedron -d 6.5 <<EOF
1
EOF
