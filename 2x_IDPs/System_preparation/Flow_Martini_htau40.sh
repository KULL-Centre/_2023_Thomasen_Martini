...
#rm -r $dir
mkdir $dir
cp Structures/two_htau40_collapsed.pdb $dir
cp Structures/two_htau40_AA.pdb $dir
cp $insane $dir
cd $dir

#Martinize
$python $martinize -f two_htau40_AA.pdb -o PRO_topol.top -x trash.pdb -ff $FF -sep -ff-dir $ffdir/martini_v3.0.0_proteins/force_fields/ -map-dir $ffdir/martini_v3.0.0_proteins/mappings/ 
rm trash.pdb
cp two_htau40_collapsed.pdb PRO_CG.pdb

$python ../change_beadtypes_moleculeitp.py molecule_0.itp
$python ../change_beadtypes_moleculeitp.py molecule_1.itp

#Put protein in box
$gmx editconf -f PRO_CG.pdb -o PRO_CG.gro -bt cubic -box 48.02 48.02 48.02 <<EOF
1
EOF
