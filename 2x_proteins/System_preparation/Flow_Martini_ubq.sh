...
#Martinize
$python $martinize -f $pdb -o PRO_topol.top -x PRO_CG.pdb -ff $FF -dssp $dssp -cys auto -elastic -ef 700.0 -el 0.5 -eu 0.9 -scfix -ff-dir $ffdir/martini_v3.0.0_proteins/force_fields/ -map-dir $ffdir/martini_v3.0.0_proteins/mappings/

$python ../change_beadtypes_moleculeitp.py molecule_0.itp
$python ../change_beadtypes_moleculeitp.py molecule_1.itp

#Put protein in box
$gmx editconf -f PRO_CG.pdb -o PRO_CG.gro -bt cubic -box 14.92 14.92 14.92 <<EOF
1
EOF

#Solvate using insane.py
$python2 $insane -f PRO_CG.gro -o PRO_SOL_IONS.gro -pbc keep -salt 0.11 -sol W -center -p PRO_topol_SOL_IONS.top
...
$python ../choose_rubber_bands_ubiquitin.py
