...
B="python2.7 $SDIR/backward.py -f $INP -raw $RAW -o $GRO -kick $KICK -sol -p $TOP -po $OTP -n $NDX -from $CG -to $AA"
echo $B; $B || exit
...
# Run
M="$gmx mdrun -deffnm $BASE -v -ntomp 1"
echo $M; $M 
...
do
  ...
  echo $G; $G || exit
  #M="$gmx mdrun -deffnm $tag -v -ntomp 1"
  echo $M; $M 
  #GRO=$tag.gro
